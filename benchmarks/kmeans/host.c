...
void run(){
  ...
  small_table_scatter("t1", elements, num_elements, dim*sizeof(T), 0, table_management);
  uint32_t data_offset = lookup_table("t1", table_management)->end; 

  // main loop
  for(int m=0; m<iter; m++){
    small_table_broadcast("t2", centroids, k, dim*sizeof(T), data_offset, table_management);
    uint32_t weights_offset = lookup_table("t2", table_management)->end; 

    handle_t* va_handle = create_handle("kmeans_funcs", 1);
    T* res = table_gen_red("t1", "t3", weights_offset, dim*sizeof(T)+sizeof(int32_t), k, va_handle, table_management, data_offset);
