...
void run(){
    ...
    //T* zip_res = (T*)malloc((uint64_t)sizeof(T)*2*nr_elements);
    T* correct_res = (T*)malloc((uint64_t)sizeof(T)*nr_elements);
    init(A, 0);
    init(B, 1);
    //zip(A, B, zip_res);
    //vec_add(zip_res, correct_res);
    ...
    small_table_scatter("t2", B, nr_elements,  sizeof(T), data_offset, table_management);
    stop(&timer, 0);
    printf("end of data transfer\n");

    handle_t* add_handle = create_handle("va_funcs", 0);
    handle_t* zip_handle = create_handle("", 2);
    data_offset = lookup_table("t2", table_management)->end;


    start(&timer, 1, 0);
    table_zip("t1", "t2", "t3", data_offset, zip_handle, table_management);
    data_offset = lookup_table("t3", table_management)->end;
    table_map("t3", "t4", data_offset, sizeof(T), add_handle, table_management, 0);
    stop(&timer, 1);
