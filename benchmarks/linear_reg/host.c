...
int main(){
  ...
  */
  ...
  small_table_scatter("t1", elements, num_elements, (dim+1)*sizeof(T), 0, table_management);
  uint32_t data_offset = lookup_table("t1", table_management)->end; 
  small_table_broadcast("t2", weights, 1, dim*sizeof(T), data_offset, table_management);
  uint32_t weights_offset = lookup_table("t2", table_management)->end; 

  handle_t* va_handle = create_handle("lin_reg_funcs", 1);

  for(int l=0; l<iter; l++){
    int64_t* res = table_gen_red("t1", "t3", weights_offset, dim*sizeof(int64_t), 1, va_handle, table_management, data_offset);
